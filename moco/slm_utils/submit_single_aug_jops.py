...
for idx in range(1, 15):  # TODO Boost to 16
...
for custom_aug_name in custom_aug_names: 
	...
	filename = '/userdata/smetzger/all_deepul_files/runs/imgnet_' + custom_aug_name + '_single_aug_test_%d.txt' %(idx)
	string = "submit_job -q mind-gpu"
	string += " -m 318 -g 4"
	string += " -o " + filename
	string += ' -n MoCo1'
	...
	string += ' --checkpoint_fp ' + str(checkpoint_fp)
	string += ' --rank 0'
	string += " --data /userdata/smetzger/data/cifar_10/ --notes 'imgnet_single_aug_%d'" %idx
	...
	# These args let us use redeuced imagenet as the xform. 
	string += ' --kfold 0' 
	string += ' --reduced_imgnet'
